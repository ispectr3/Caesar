...
const SPECIFIC_BLOCKS = {
  ...
  '/cpf': {
    how: 'Valida o CPF usando o algoritmo de dígitos verificadores da Receita Federal. Extrai a região de origem (os três primeiros dígitos indicam o estado de emissão) e verifica exposição em vazamentos.',
    interpret: 'Um CPF válido não significa que pertence à pessoa alegada. A região de emissão pode revelar inconsistências geográficas. A presença em vazamentos indica que dados sensíveis associados estão circulando na dark web.',
  },
  '/cnpj': {
  ...
  '/leaklooker': {
    how: 'Gera links diretos para o Shodan e ZoomEye buscando serviços expostos (Elasticsearch, MongoDB, Redis, etc.) associados ao alvo. A varredura real acontece no Shodan/ZoomEye.',
    interpret: 'Um banco de dados ElasticSearch exposto na porta 9200 sem autenticação é um vazamento crítico. MongoDB na porta 27017 frequentemente contém dados de produção. Documente e notifique responsavelmente.',
  },
  '/favicon': {
  ...
  '/password': {
    ...
    interpret: 'Use para criar senhas seguras para operações encobertassas. Senhas de pelo menos 20 caracteres com símbolos são resistentes a ataques de força bruta com hardware de última geração.',
  },
  '/crypto': {
...
async function main() {
  ...
  await Promise.all(tsxFiles.map(async (file) => {
    ...
    if (!block) return; // No specific description for this module

    const filePath = path.join(routesDir, file);
    try {
      const content = await fs.readFile(filePath, 'utf8');

      // Replace the generic auto-generated block with the specific one
      const genericBlockRegex = /\s*\{\/\* Bloco Explicativo Automático \*\/\}\s*<div className="mt-8">\s*<ResultCard title="Como funciona & Próximos Passos">[\s\S]*?<\/ResultCard>\s*<\/div>/g;

      const specificBlock = `
      {/* Bloco Tático Específico */}
      <div className="mt-8">
        <ResultCard title="Como funciona & Como Interpretar">
          <div className="space-y-3 font-mono text-[11px] leading-relaxed text-muted-foreground">
            <p>
              <strong className="text-primary">Como funciona:</strong> ${block.how}
            </p>
            <p>
              <strong className="text-primary">Como interpretar o resultado:</strong> ${block.interpret}
            </p>
          </div>
        </ResultCard>
      </div>
    `;
      ...
      const newContent = content.replace(genericBlockRegex, specificBlock);

      if (content !== newContent) {
        await fs.writeFile(filePath, newContent, 'utf8');
        changedCount++;
        console.log(`Updated specific description for: ${file}`);
      } else {
        // Try to inject it before </SiteLayout> if there's no generic block
        if (!content.includes('Bloco Tático Específico')) {
          const injected = content.replace(/(\s*<\/SiteLayout>)/, specificBlock + '\n    </SiteLayout>');
          if (injected !== content) {
            await fs.writeFile(filePath, injected, 'utf8');
            changedCount++;
            console.log(`Injected description for: ${file}`);
          }
        }
      }
    } catch (e) {
  ...
  console.log(`\nCOMPLETED: Updated ${changedCount} modules with specific descriptions.`);
}
